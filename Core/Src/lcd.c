 ...
 *          of the LCD of the STM32F4xx device.
**/
#include <lcd.h>
#define LCD_SPI hspi3
static pFONT *LCD_AsciiFonts;       // Pointer to the ASCII fonts


/*
 * SPI Screens require configuring coordinates before writing the GRAM,
 * which could be slowly if the program writes the GRAM pixel-by-pixel.
 * Creating a buffer would increase the performance.
 *
 * BUFF_SIZE specifies the size of the buffer. If you want to display a
 * 32*32 character, * you will need 32*32*2 = 2048 bytes (each pixel
 * occupies two bytes).
 */
uint16_t LCD_Buff[BUFF_SIZE];       // LCD Buffer
...
LCD_Init (void)
{
  HAL_Delay (100);               // Wait for the LCD to reset
  LCD_CS_L;                      // Pull the chip select low, start communication
  LCD_WriteCommand (0x36);       // Cmd: Set method to access GRAM
  LCD_WriteData_8bit (0x00);     // Scan mode: Up to Dn, Lft to Rt, RGB

  LCD_WriteCommand (0x3A);       // Cmd: Set pixel format (12/16/18-bit color)
  LCD_WriteData_8bit (0x05);     // Set to 16-bit

/* Following are the voltage settings as provided by the manufacturer */
  ...
  HAL_Delay (500);               // 需要等待120ms，让电源电压和时钟电路稳定下来
...
// 以下进行一些驱动的默认设置
  ...
  LCD_DisplayText (0, 0, "LCD Init OK!");
// 全部设置完毕之后，打开背光
...
LCD_DisplayText (uint16_t x, uint16_t y, char *pText)
{
  ...
  while (*pText != 0)  // 判断是否为空字符
    {
      if (*pText <= 0x7F)    // 判断是否为ASCII码，忽略非ASCII字符
        {
